...
class Sequential(nn.Sequential):
    ...
    def grad(self, x1, x2=None):
        x1s = torch.split(x1,1)
        x1c = torch.cat(x1s)
        x1c = x1c.requires_grad_(True)
        with torch.enable_grad():
            if x2 is None:
                # get a gradient when one argument is given.
                h = self(x1c)
                grad = torch.autograd.grad(h.sum(), (x1c,), create_graph=True)[0]
                return grad
            else:
    ...
    def jacgrad(self, x1, x2=None):
        with torch.enable_grad():
            if x2 is None:
                # If only one argument is given, return the Hessian
                x1 = x1.requires_grad_(True)
                g = self.grad(x1)
                hess1 = torch.autograd.grad(torch.split(g[:,0],1), (x1,), retain_graph=True)[0]
                hess2 = torch.autograd.grad(torch.split(g[:,1],1), (x1,), retain_graph=True)[0]
                hess = torch.stack((hess1, hess2),1)
                return hess
            else:
                # Return the Jacobian of the discrete gradient w.r.t. the second argument (x2)
                x2 = x2.requires_grad_(True)
                g = self.grad(x1, x2)
                jacgrad1 = torch.autograd.grad(torch.split(g[:,0],1), (x2,), retain_graph=True)[0]
                jacgrad2 = torch.autograd.grad(torch.split(g[:,1],1), (x2,), retain_graph=True)[0]
                jacgrad = torch.stack((jacgrad1, jacgrad2),1)
                return jacgrad
