...
class DGNet(torch.nn.Module):
    ...
    def jacgrad(self, x1, x2=None):
        return self.net.jacgrad(x1, x2)
    ...
    def time_derivative(self, x1, x2=None):
        S = self.S_t
        dH = self.net.grad(x1, x2)
        vector_field = (S @ dH.T).T
        return vector_field

    def discrete_time_derivative_dg3(self, x1, dt=None, x2=None):
        S = self.S_t
        dH = self.net.grad(x1)
        vector_field = (S @ dH.T).T
        if x2==None:
            ...
            z = x1 + 2/3*dt*vector_field
            HH = self.net.jacgrad(x1) # the Hessian of H at y0
            SH = S @ HH
            SHS = SH @ S
            SHSHS = SH @ SHS
            DDGz = self.net.jacgrad(x1,z) # Jacobian of discrete gradient DGH(y0,z)  w.r.t. z
            Q = .5*(torch.transpose(DDGz,1,2)-DDGz)
            SQ = S @ Q 
            SQS = SQ @ S
            St = S + dt*SQS - 1/12*dt**2*SHSHS
            dH = self.net.grad(x1, x2).unsqueeze(2)
            vector_field = (St @ dH).squeeze(2)
            return vector_field
        
    def discrete_time_derivative_dg4(self, x1, dt=None, x2=None):
        S = self.S_t
        dH = self.net.grad(x1)
        vector_field = (S @ dH.T).T
        if x2==None:
            ...
            xh = x1 + .5*dt*vector_field
            dH = self.net.grad(xh)
            z = x1 + (.75*dt*(S @ dH.T)).T
            HH = self.net.jacgrad(xh) # the Hessian of H at xh
            SH = S @ HH 
            SHS = SH @ S 
            SHSHS = SH @ SHS 
            DDGz = self.net.jacgrad(x1,z) # Jacobian of discrete gradient DGH(y0,z)  w.r.t. z
            Q = .5*(torch.transpose(DDGz,1,2)-DDGz)
            SQ = S @ Q #torch.mm(S,Q)
            SQS = SQ @ S # torch.mm(SQ,S)
            St = S + 8/9*dt*SQS - 1/12*dt**2*SHSHS
            dH = self.net.grad(x1, x2).unsqueeze(2) # append a dimension by unsqueeze
            vector_field = (St @ dH).squeeze(2)
            return vector_field
        
    def discrete_time_derivative_rk4(self, x1, dt=None, x2=None):
        S = self.S_t
        f = lambda x: (S @ self.net.grad(x).T).T
        vector_field = 1/6*(f(x1)+2*f(x1+.5*dt*f(x1))+2*f(x1+.5*dt*f(x1+.5*dt*f(x1)))+f(x1+dt*f(x1+.5*dt*f(x1+.5*dt*f(x1)))))
        return vector_field
    ...
    def discrete_time_derivative(self, x1, *, dt=None, x2=None, xtol=1.0e-12, eval_mode=False):
        ...
        if solver in SOLVER_LIST_ADDITIONAL_IMPLICIT:
            if x2 is None:
                ...
                # Get discrete gratient using Eq. (6) in (Matsubara 2019) when x2 is given; otherwise implicitly.
