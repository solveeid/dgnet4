...
from data_pend import get_dataset
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.show()
    #fig.savefig('solvers_pend.eps', format='eps', bbox_inches='tight')
    ...
    plt.ylabel("Energy error", fontsize=12)
    plt.title("Energy errors", fontsize=14)
    plt.legend()
    plt.show()
    #fig.savefig('energyerrors_pend.eps', format='eps', bbox_inches='tight')
    ...
    plt.loglog(dt, .7*dt**2, 'k--', linewidth=1)
    plt.loglog(dt, .2*dt**3, 'k--', linewidth=1)  
    plt.loglog(dt, 1*dt**4, 'k--', linewidth=1)  
    plt.xlabel("Step size", fontsize=12)
    plt.ylabel("Error at $t=1$", fontsize=12)
    plt.title("Order plot", fontsize=14)
    plt.legend()
    plt.show()
    #fig.savefig('orderplot_pend.eps', format='eps', bbox_inches='tight')