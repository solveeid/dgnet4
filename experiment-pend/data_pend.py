...
def hamiltonian_fn(coords):
    ...
    H = 2*m*g*l*(1-np.cos(q)) + .5*l**2/m*p**2 # pendulum hamiltonian
    return H
...
def get_trajectory(t_span=[0,3], timescale=15, radius=None, y0=None, noise_std=0.1, **kwargs):
    ...
    if radius is None:
        radius = np.random.rand() + 2.0 # sample a range of radii
    y0 = y0 / np.sqrt((y0**2).sum()) * radius ## set the appropriate radius

    pend_ivp = solve_ivp(fun=dynamics_fn, t_span=t_span, y0=y0, t_eval=t_eval, rtol=1e-10, **kwargs)
    q, p = pend_ivp['y'][0], pend_ivp['y'][1]
    dydt = [dynamics_fn(None, y) for y in pend_ivp['y'].T]
